...
All modules require permissions to be setup in order to use them, so its good to set this up first. Run the `/config` command and click on the menu to reveal the modules. Select `Configure Permissions`. This will bring you to the Permissions embed. Select the drop down menu's once again to select roles for both Staff & Management. Each of those roles will have a different set of permissions, please take a look at the chart below to see which roles need what permissions.
